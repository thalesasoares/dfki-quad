// Stock gait-sequencer plugins (issue #8, M2.3; bio_gait added by #16, M4.1).
//
// Thin adapters (plugin_lifecycle.md §4, Option B) around the existing
// SimpleGaitSequencer / AdaptiveGaitSequencer / BioGaitSequencer. Each holds a
// unique_ptr to the untouched algorithm class, builds it inside Init from
// StageInit, and forwards the frozen GaitSequencerInterface methods to it. No
// algorithm code changes, so "no intentional behavior change" (issue #8
// acceptance criterion 3) is auditable by reading this file against
// GetGaitSequencerFromParams in mit_controller_node.cpp — the Init bodies are
// that factory's two branches with get_parameter(x).as_T() replaced by
// Require<T>/GetOr<T>.
//
// BioGaitSequencerPlugin (#16, M4.1) is the exception to that provenance: the
// pre-plugin factory had no "Bio" branch, so the class was compiled but
// unreachable from parameters — which is exactly what #16 fixes. Its Init is
// therefore *new* code rather than a relocation, but it is deliberately written
// as SimpleGaitSequencerPlugin::Init minus the Gait construction: the shared
// gait-sequencer keys must keep one spelling and one required-vs-defaulted
// split across all three wrappers.
...
// exactly as the host read them (mit_controller_node.cpp:641-648). `flat` outlives
// the copy into the returned value array, so the Maps are only a view during
// construction.
//
// Reading and validating the key lives here rather than in each Init so that the
// one parameter all three sequencers take keeps one spelling, one strictness and
// one error message. #16 (M4.1) added the third caller; up to then the four lines
// were duplicated in the two wrappers below.
std::array<const Eigen::Vector3d, N_LEGS> RequireShoulderPositions(const StageInit& init) {
  const std::vector<double> flat = Require<std::vector<double>>(init, "gs_shoulder_positions");
  if (flat.size() != static_cast<size_t>(N_LEGS * 3)) {
...
class SimpleGaitSequencerPlugin final : public StagePlugin<GaitSequencerInterface> {
 public:
  void Init(StageInit init) override {
    const auto shoulder_positions = RequireShoulderPositions(init);
...
/**
 * Stock plugin wrapping BioGaitSequencer (issue #16, M4.1).
 *
 * Unlike the two above there is no host factory branch to mirror: the class was
 * compiled but never constructed from parameters, which is the gap this plugin
 * closes. Init is therefore SimpleGaitSequencerPlugin::Init with the Gait
 * construction removed — the sequencer picks its own gait from BioGaitDatabase by
 * Froude number, so it takes no gait parameters and adds no `bio_gait_sequencer.*`
 * key. Note the constructor's argument order: `k` comes first, before the
 * shoulder positions.
 */
class BioGaitSequencerPlugin final : public StagePlugin<GaitSequencerInterface> {
 public:
  void Init(StageInit init) override {
    const auto shoulder_positions = RequireShoulderPositions(init);
 ...
 private:
  std::unique_ptr<BioGaitSequencer> impl_;
};
